namespace CsvCleanToUSCharset
{
    ...
    {
        internal static async Task<bool> ProcessFileAsync(FileInfo file, bool skipFirstRow, ConcurrentDictionary<int, string> logInfo, CancellationToken token)
        {
            object mylock = new ();
            try
            {
                ...
                await foreach (var line in File.ReadLinesAsync(file.FullName, token))
                {
                    ...
                    addresses[i] = line;
                    if (i > 0 || !skipFirstRow) { SubstituteChars(ref addresses[i]); }
                }
    ...
    private static void SubstituteChars(ref string origin)
    {
        if (origin == null) { return; }

        bool specialCountry = false;

        if (origin.Contains("\"DE\"") || origin.Contains("\"AT\"") || origin.Contains("\"CH\""))
            specialCountry = true;
